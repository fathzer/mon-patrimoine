type ContentProvider = (args: unknown) => string;

interface HelpPopoverOptions {
  title?: string;
...
function onDocumentClick(e: Event): void {
  ...
  const trigger = target.closest('.help-popover') as HTMLElement | null;
  if (trigger) {
    e.preventDefault();
    show(trigger);
    return;
  ...
  if (target.closest('.help-popover-card')) {
    return;
  }
