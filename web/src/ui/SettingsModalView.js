...
export class SettingsModalView {
  ...
  _bindEvents() {
    ...
    const alternateChildrenInput = form.querySelector('#alternate-children-input');
    const singleParentInput = form.querySelector('#single-parent-input');
    const taxableIncomeInput = form.querySelector('#taxable-income-input');
    const partsDisplay = form.querySelector('#parts-display');
    const reductionCeilingWrapper = form.querySelector('#reduction-ceiling-wrapper');
    ...
    const updateFiscalSummary = () => {
      ...
      const taxResult = TaxCalculator.computeFinalTax(taxableIncome, statusSelect.value, extraParts, summary.halfPartReductionCeiling);
    ...
    form?.addEventListener('submit', (e) => {
      ...
      const taxableIncome = Number.parseFloat(formData.get('taxableIncome') || 0);
      const maritalStatus = formData.get('maritalStatus');
      const childrenCount = Number.parseInt(formData.get('childrenCount') || 0, 10);
      const alternateChildrenCount = Number.parseInt(formData.get('alternateChildrenCount') || 0, 10);
