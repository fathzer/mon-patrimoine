...
import type { Household, HouseholdData, MaritalStatus } from './Household.js';
...
export class TaxCalculator {
   ...
   */
  private static _computeChildrenImpact(household: Household | HouseholdData | undefined): ChildrenImpact {
    ...
    // Widowed with dependent children keep the deceased spouse's part,
    // capped at the general half-part rate like children parts.
    const widowParts = maritalStatus === 'widowed' && totalChildren > 0 ? 1 : 0;

    const { bonus, discount } = this._computeStatusImpact(household, maritalStatus, childrenCount, totalChildren);

    const extraParts = childParts + widowParts + bonus;
    const ceiling = (childParts + widowParts) * 2 * childCeiling + discount;
  ...
  /**
   * Computes the bonus parts and specific ceilings linked to the household
   * situation: single parent (case T), case L and the widowed complementary
   * reduction.
   */
  private static _computeStatusImpact(
    household: Household | HouseholdData | undefined,
    ...
    // Single parent extra part (case T): half if at least one exclusive child,
    // quarter otherwise. Only single/divorced taxpayers can claim it.
    const singleParentActive = isSingleParent && maritalStatus === 'single';
    if (singleParentActive && childrenCount > 0) {
      bonus = 0.5;
      discount = singleParentCeiling;
    } else if (singleParentActive && totalChildren > 0) {
      bonus = 0.25;
      discount = singleParentCeiling / 2;
    }

    // Case L (ex-parent isolé): extra half part for a single or widowed person
    // with no dependent child who raised a child alone for at least five years.
    if (household?.caseL && maritalStatus !== 'married' && totalChildren === 0) {
      bonus += 0.5;
      discount += FISCAL_RATES.EXTRA_PARTS.CEILING.CASE_L;
    }

    // Widowed taxpayers with at least one child in exclusive custody get a
    // complementary post-ceiling reduction (reduc_postplafond_veuf), which is
    // mathematically equivalent to a ceiling increase.
    if (maritalStatus === 'widowed' && childrenCount > 0) {
      discount += FISCAL_RATES.EXTRA_PARTS.CEILING.WIDOW_POST_CAP_REDUCTION;
    }
