...
const UC_SOCIAL_RATE = FISCAL_RATES.OLD_CSG_CRDS;
const PFU_BEFORE_8Y = FISCAL_RATES.PFU_IR_RATE;
const PFU_AFTER_8Y_PRE_2017 = 0.075;
const PFU_AFTER_8Y_POST_2017_LOW = 0.075;
const PFU_AFTER_8Y_POST_2017_HIGH = 0.128;
const PREMIUM_150K_THRESHOLD = 150000;
...
export class LifeInsuranceModule extends BasePlacement {
  ...
  _computePost8YearsImposition(fiscalProfile, totalGain, ucShare, preShare, postShare) {
    ...
    let postImposition;
    if (post2017Premiums <= PREMIUM_150K_THRESHOLD) {
      postImposition = postTaxableGain * PFU_AFTER_8Y_POST_2017_LOW;
    } else {
      const firstFraction = Math.min(1, PREMIUM_150K_THRESHOLD / post2017Premiums);
